...
const CommentAndLike = async function (session, accountId, text) {
    ...
    if (result.length > 0) {
        ...
        const printFollow = Follow ? chalk`{green SUKSES}` : chalk`{red GAGAL}`;
        const printComment = Comment ? chalk`{green SUKSES}` : chalk`{red GAGAL}`;
        const printLike = Like ? chalk`{green SUKSES}` : chalk`{red GAGAL}`;
    ...
    try {
        ...
        do {
            ...
            for (let i = 0; i < TargetResult.length; i++) {
                ...
                console.log(chalk`{bold.red \n [*]Delay ${sleep}[Miliseconds]}{bold.yellow [Username : ${Usernames}]}{bold.magenta [Followers :[${getpiceksu.followers}]/[Following :[${getpiceksu.following}]}{bold.green [Username Target : ${TargetUsername}] [<] }\n`);
                await delay(sleep);
            }
...
console.log(chalk`{bold.green
  Ξ TITLE  : FFT [FOLLOW]-[LIKE]-[COMMENT] TARGET USER!
  Ξ CODE   : CYBER SCREAMER CCOCOT (ccocot@bc0de.net)
  Ξ UPDATE : ADD ITTYW by @novalrahadian_
  Ξ STATUS : ITTYW : {bold.yellow Supported!}}
      `);
//mcakz.co
